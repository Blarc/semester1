...
public class naloga3_ver2 {
	...
	public static void fri(LinkedList list, LinkedListElement atm, int idealStart, int razdalja, int stevec, int[][] tab, int index) {
		...
		if (atm.next == null) {
			frej = Integer.MAX_VALUE;
		}
		
		
		//preverimo vse bloke ki jih lahko poravnamo
		for (LinkedListElement i = list.first; i.next != null; i = i.next) {
			Blok novBlok = (Blok)i.next.element;
			
			//ce blok ni poravnan
			if (!novBlok.poravnan) {
				//ce je atm
				if (novBlok.id == atmBlok.id || stevec == 1) {
					novBlok.poravnan = true;
					int oldOne = tab[index][0];
					int oldTwo = tab[index][1];
	...
	public static void main(String[] args) throws IOException {
		...
		while ((readLine = br.readLine()) != null) {
			...
			//create empty blocks
			/*if (oldEnd+1 != start) {
				novBlok = new Blok(0, oldEnd+1, start-1);
